...
developomentChains.includes(network.name)
    ? describe("FundIndia", async function () {
          ...
          describe("withdraw", async function () {
              ...
              it("should Withdarw the ETH from a single funder", async function () {
                  //Arrange
                  const beforeContractBalance =
                      await ethers.provider.getBalance(fundIndia.target)
                  ...
                  assert.equal(
                      (
                          beforeContractBalance + beforeDeployerBalance
                      ).toString(),
              ...
              it("should withdaw eth from multiple funders", async function () {
                  ...
                  for (let i = 1; i < 6; i++) {
                      const contractConnectedAccount = fundIndia.connect(
                          accounts[i]
                      )
                      await contractConnectedAccount.fund({ value: sendValue })
                  }
                  const beforefundIndiaBalance =
                      await ethers.provider.getBalance(fundIndia.target)
                  const beforeDeployerBalance =
                      await ethers.provider.getBalance(deployer)
                  ...
                  assert.equal(
                      (
                          beforefundIndiaBalance + beforeDeployerBalance
                      ).toString(),
                      (afterDeployerBalance + gasCost).toString()
                  )
                  ...
                  for (let i = 1; i < 6; i++) {
                      assert.equal(
                          await fundIndia.getAddressToAmountFunded(
                              accounts[i].address
                          ),
                          0
                      )
    ...
    : describe.skip("These Tests are not for production")
